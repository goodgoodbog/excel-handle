# -*- coding=utf-8-*-
# @Time : 2024/9/15 16:56
# @Author : BQ
# @File : supplement_plus.py
# @Software: PyCharm
# -*- coding=utf-8-*-
# 功能3plus：回填运单号订单列表处理
import os
...
class Supplement_plus:
    def __init__(self, root):
        self.use = all_use.Use(root)
        self.d1 = tk.Label(root, text='功能3plus：补充信息：')
        self.d2 = tk.Label(root, text='E键发运单:')
        self.e1 = tk.Entry(root, name='supplement_plus_e1', width=56, highlightthickness=1.5, highlightcolor='blue',
                           validate="key",
                           relief='sunken',
        ...
        self.e2 = tk.Entry(root, width=56, highlightthickness=1.5, highlightcolor='blue', name='supplement_plus_e2',
                           validate="key",
                           relief='sunken',
                           validatecommand=(self.use.check_open_file1, '%P', '%W'))
        self.four_choose = ttk.Button(root, text='选择文件', command=self.four_choose_flie9)
        self.d4 = tk.Label(root, text='需要回填的订单号:')
        self.e3 = tk.Entry(root, width=56, highlightthickness=1.5, highlightcolor='blue', name='supplement_plus_e3',
                           validate="key",
                           relief='sunken',
                           validatecommand=(self.use.check_open_file1, '%P', '%W'))
    ...
    def work(self):
        ...
        for i in range(1, column1 + 1):
            ...
            # '运单号'改为'4px单号'
            if sheet1.cell(1, i).value == '4px单号':
                yun_dan_hao1 = i
                break
        # 检查文件中是否存在某列
        if Order_Numbe2 == -1:
